    ...
    <main className="max-w-[1280px] mx-auto px-8 py-8">
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50" onClick={() => { if (!newKeyResult) setShowCreate(false); }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md mx-4 shadow-xl border border-[#ECEFF3]" onClick={e => e.stopPropagation()}>
            {newKeyResult ? (
              <div>
                <h3 className="text-lg font-semibold text-[#111827] mb-2">{t('createSuccess.title')}</h3>
                <p className="text-sm text-[#6B7280] mb-4">{t('createSuccess.warning')}</p>
                <div className="bg-[#F8FAFC] border border-[#ECEFF3] rounded-xl p-3 mb-4 font-mono text-sm break-all select-all">{newKeyResult}</div>
                <div className="flex gap-2">
                  <button onClick={() => copyKey('new', newKeyResult)} className="flex-1 h-10 bg-[#111827] hover:bg-[#1f2937] text-white rounded-lg text-sm font-medium">
              ...
              <div>
                <h3 className="text-lg font-semibold text-[#111827] mb-4">{t('createDialog.title')}</h3>
                <label className="block text-sm font-medium text-[#6B7280] mb-1.5">{t('createDialog.nameLabel')}</label>
                <input value={newKeyName} onChange={e => setNewKeyName(e.target.value)}
                  className="w-full h-10 px-3 border border-[#ECEFF3] rounded-xl text-sm focus:outline-none focus:border-[#111827] transition-colors mb-2"
                  placeholder={t('createDialog.namePlaceholder')} />
                <label className="block text-sm font-medium text-[#6B7280] mb-1.5">{t('createDialog.rateLimitLabel')}</label>
      ...
      {isLoading_ ? (
        ...
        <div className="bg-white rounded-2xl border border-[#ECEFF3] shadow-sm py-20 px-6 text-center">
          <KeySvg className="w-16 h-16 mx-auto mb-5 text-gray-200" />
          <h3 className="text-lg font-semibold text-[#111827] mb-1">{t('emptyState.title')}</h3>
          <p className="text-sm text-[#6B7280] mb-6">{t('emptyState.message')}</p>
