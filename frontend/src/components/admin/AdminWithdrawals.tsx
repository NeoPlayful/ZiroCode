          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 dark:border-[#303033]">
                <th className="text-left px-6 py-4 text-xs font-medium text-gray-500 dark:text-[#98989D] uppercase tracking-wider">{t('withdrawals.table.user')}</th>
                <th className="text-left px-6 py-4 text-xs font-medium text-gray-500 dark:text-[#98989D] uppercase tracking-wider">{t('withdrawals.table.amount')}</th>
                <th className="text-left px-6 py-4 text-xs font-medium text-gray-500 dark:text-[#98989D] uppercase tracking-wider">{t('withdrawals.table.status')}</th>
                <th className="text-left px-6 py-4 text-xs font-medium text-gray-500 dark:text-[#98989D] uppercase tracking-wider">{t('withdrawals.table.createdAt')}</th>
                <th className="text-right px-6 py-4 text-xs font-medium text-gray-500 dark:text-[#98989D] uppercase tracking-wider">{t('withdrawals.table.actions')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 dark:divide-[#303033]/50">
              ...
              {withdrawals.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-400 dark:text-[#6E6E73]">
                    {t('withdrawals.empty')}
                  </td>
                </tr>
              )}
...
function WithdrawalRow({ withdrawal, onApprove }: any) {
  ...
  const getStatusColor = (status: string) => {
    if (status === 'APPROVED') return 'bg-green-50 dark:bg-[#30D158]/10 text-green-700 dark:text-[#30D158]'
    if (status === 'PENDING') return 'bg-yellow-50 dark:bg-[#FF9F0A]/10 text-yellow-700 dark:text-yellow-300'
    if (status === 'REJECTED') return 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-[#FF453A]'
    ...
    <tr className="hover:bg-gray-50/50 dark:hover:bg-[#242426]/50 transition-colors">
      <td className="px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-medium text-gray-900 dark:text-[#E5E5E7]">{withdrawal.user?.name || t('withdrawals.unnamed')}</div>
            <div className="text-sm text-gray-500 dark:text-[#98989D]">{withdrawal.user?.email}</div>
          </div>
        </div>
      </td>
      <td className="px-6 py-4">
        <div className="text-lg font-semibold text-gray-900 dark:text-[#E5E5E7]">¥{Number(withdrawal.amount).toFixed(2)}</div>
      </td>
      <td className="px-6 py-4">
        ...
        <div className="flex items-center justify-end gap-2">
          {withdrawal.status === 'PENDING' && (
            <button
              ...
              className="px-3 py-1.5 text-xs bg-green-50 dark:bg-[#30D158]/10 hover:bg-green-100 dark:hover:bg-green-900/50 text-green-700 dark:text-[#30D158] rounded-lg transition-colors font-medium"
            >
